import React from 'react'
import Footer from './Footer'
import Header from './Header'

const Team = () => {
  return (
    <>
    <Header/>
    <div className="more-projects-wrapper in-page">
      <div className="project-gallery-wrapper">
        <div className="grid-wrapper projects-search">
          <div
            id="w-node-_397244f1-6d58-8f51-019c-f7e5a2b1e41a-f9270e17"
            className="project-discovery-wrapper"
          >
            <div
              id="w-node-_849e754c-a02b-b240-b303-2bf5e8a0db81-f9270e17"
              className="project-discovery-cover two"
            />
            <div
              id="w-node-a30333d2-032c-03e5-f1b1-a5d31eb7f5e4-f9270e17"
              className="gallery-items-wrapper"
            >
              <div className="project-discovery-list">
                <div className="gallery-heading">
                  <h3 className="heading-2">Explore our work</h3>
                </div>
                <div
                  id="scrollbar"
                  className="project-search-collection w-dyn-list"
                >
                  <div role="list" className="project-search-items w-dyn-items">
                    <div
                      role="listitem"
                      className="project-search-collection-item w-dyn-item"
                    >
                      <a
                        href="#"
                        className="menu-list-item discovery-item w-inline-block"
                      >
                        <div
                          id="w-node-c47dbc6c-9b93-d53a-3ac5-94688c374a85-f9270e17"
                          className="stacked-description short"
                        >
                          <div className="list-item-text subtitle grey w-dyn-bind-empty" />
                          <div className="list-item-text w-dyn-bind-empty" />
                        </div>
                        <img
                          src="images/arrow-right48x482x.svg"
                          loading="lazy"
                          id="w-node-_6ad8919c-2309-2a37-80a2-1e0756464dfe-f9270e17"
                          alt="arrow"
                          className="list-item-arrow"
                        />
                      </a>
                    </div>
                  </div>
                  <div className="w-dyn-empty">
                    <div>No items found.</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div
            id="w-node-_24909537-2aef-fa73-cf08-5d19c7632e15-f9270e17"
            className="project-discovery-cover one"
          />
          <div className="project-gallery-overlay" />
        </div>
        <div className="more-projects-wrapper">
          <div className="grid-wrapper fixed">
            <div
              id="w-node-a30333d2-032c-03e5-f1b1-a5d31eb7f5f0-f9270e17"
              data-w-id="a30333d2-032c-03e5-f1b1-a5d31eb7f5f0"
              className="fly-out-button on-click"
            >
              <div
                id="w-node-a30333d2-032c-03e5-f1b1-a5d31eb7f5f1-f9270e17"
                className="projects-lottie-wrapper"
              >
                <div
                  data-is-ix2-target={1}
                  className="projects-lottie"
                  data-w-id="a30333d2-032c-03e5-f1b1-a5d31eb7f5f2"
                  data-animation-type="lottie"
                  data-src="documents/Project-open-dropdown.json"
                  data-loop={0}
                  data-direction={1}
                  data-autoplay={0}
                  data-renderer="svg"
                  data-default-duration="2.4791666666666665"
                  data-duration={0}
                  data-ix2-initial-state={0}
                />
              </div>
              <div
                id="w-node-a30333d2-032c-03e5-f1b1-a5d31eb7f5f3-f9270e17"
                className="subtitle"
              >
                Discover projects
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="grid-wrapper project-discovery-grid">
        <div
          id="w-node-c1267971-e98a-4887-2b61-4f4973f4569f-f9270e17"
          className="discovery-button-wrapper"
        >
          <div
            id="w-node-_93ff6921-d28d-6640-4312-da6eaaed889e-f9270e17"
            className="fly-out-button-wrapper"
          >
            <div
              data-w-id="fc177a8e-c247-e188-8355-4c56f9270e19"
              className="fly-out-button off-click"
            >
              <div
                id="w-node-fc177a8e-c247-e188-8355-4c56f9270e1a-f9270e17"
                className="projects-lottie-wrapper"
              >
                <div
                  data-is-ix2-target={1}
                  className="projects-lottie"
                  data-w-id="fc177a8e-c247-e188-8355-4c56f9270e1b"
                  data-animation-type="lottie"
                  data-src="documents/Project-open-dropdown.json"
                  data-loop={0}
                  data-direction={1}
                  data-autoplay={0}
                  data-renderer="svg"
                  data-default-duration="2.4791666666666665"
                  data-duration={0}
                  data-ix2-initial-state={0}
                />
              </div>
              <div
                id="w-node-fc177a8e-c247-e188-8355-4c56f9270e1c-f9270e17"
                className="subtitle"
              >
                Discover projects
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div className="page-wrapper">
      <div data-w-id="256f98c7-18a8-c7d5-c690-4bfe1bbed2c5" className="page-hero">
        <div className="wrapper">
          <div className="inner-page-hero-wrapper">
            <div id="w-node-_256f98c7-18a8-c7d5-c690-4bfe1bbed2c8-1300b8e6">
              <div className="subtitle white">Our team</div>
            </div>
            <div id="w-node-_256f98c7-18a8-c7d5-c690-4bfe1bbed2cb-1300b8e6">
              <div className="clip">
                <h1
                  style={{
                    WebkitTransform:
                      "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                    MozTransform:
                      "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                    msTransform:
                      "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                    transform:
                      "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                    opacity: 0
                  }}
                  className="xxl-heading one"
                >
                  Group of inside
                </h1>
              </div>
              <div className="clip">
                <h1
                  style={{
                    WebkitTransform:
                      "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                    MozTransform:
                      "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                    msTransform:
                      "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                    transform:
                      "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                    opacity: 0
                  }}
                  className="xxl-heading two"
                >
                  interior innovators.
                </h1>
              </div>
            </div>
            <div id="w-node-_256f98c7-18a8-c7d5-c690-4bfe1bbed2d0-1300b8e6">
              <div className="logos-headline">
                <div className="subtitle small white">Meet the team</div>
                <div className="horizontal-line white" />
              </div>
            </div>
          </div>
          <div
            data-w-id="256f98c7-18a8-c7d5-c690-4bfe1bbed2d5"
            className="hover-link-wrapper"
          >
            <a href="#scroll" className="hover-link w-inline-block">
              <img
                src="images/arrow-down24x242x.png"
                loading="lazy"
                width={24}
                alt="hi"
                className="scroll-icon"
              />
            </a>
          </div>
        </div>
        <div className="background-cover">
          <div className="hero-background-image team-background">
            <div className="image-overlay dark-overylay" />
          </div>
        </div>
      </div>
      <div
        id="scroll"
        data-w-id="b1309089-a2bc-3eba-7f02-d532a9467d82"
        className="section no-verticle-padding"
      >
        <div className="wrapper">
          <div className="page-info-ribbon">
            <div
              id="w-node-d5cbc97f-6c81-0aa7-94f1-53f10cb27f1c-1300b8e6"
              data-w-id="d5cbc97f-6c81-0aa7-94f1-53f10cb27f1c"
              className="page-info"
            >
              <div
                id="w-node-b1309089-a2bc-3eba-7f02-d532a9467d85-1300b8e6"
                className="banner-items"
              >
                <div className="banner-scrolling-wrapper">
                  <div className="scrolling-text">
                    <div className="subtitle">Jessica Point</div>
                    <div className="dark-dot" />
                    <div className="subtitle">RYAN BASER</div>
                    <div className="dark-dot" />
                    <div className="subtitle">CARRIE VATH</div>
                    <div className="dark-dot" />
                    <div className="subtitle">Jessica Point</div>
                    <div className="dark-dot" />
                    <div className="subtitle">RYAN BASER</div>
                    <div className="dark-dot" />
                    <div className="subtitle">CARRIE VATH</div>
                    <div className="dark-dot" />
                    <div className="subtitle">Jessica Point</div>
                    <div className="dark-dot" />
                    <div className="subtitle">RYAN BASER</div>
                    <div className="dark-dot" />
                    <div className="subtitle">CARRIE VATH</div>
                    <div className="dark-dot" />
                    <div className="subtitle">Jessica Point</div>
                    <div className="dark-dot" />
                    <div className="subtitle">RYAN BASER</div>
                    <div className="dark-dot" />
                    <div className="subtitle">CARRIE VATH</div>
                    <div className="dark-dot" />
                    <div className="subtitle">Jessica Point</div>
                    <div className="dark-dot" />
                    <div className="subtitle">RYAN BASER</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="section no-top-padding">
        <div className="wrapper">
          <h1 className="heading-53">Meet Out Team</h1>
          <div className="w-row">
            <div className="w-col w-col-6">
              <div className="card-hover-item">
                <div className="card-circle" />
                <div className="minimal-card">
                  <img src="images/maf.avif" loading="lazy" alt="madhan" />
                  <div className="category-description">
                    <div className="subtitle">
                      <strong>Madhan Kumar K, MS, P.E.</strong>
                    </div>
                    <div className="body-display">Partner</div>
                  </div>
                </div>
              </div>
            </div>
            <div className="w-col w-col-6">
              <div className="card-hover-item">
                <div className="card-circle" />
                <div className="minimal-card">
                  <img src="images/sri.avif" loading="lazy" alt="sridhar" />
                  <div className="category-description">
                    <div className="subtitle">
                      <strong>Sridhar Kethu P.E.</strong>
                    </div>
                    <div className="body-display">Associate Partner</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div
        id="scroll"
        data-w-id="dc95005d-93c8-7939-5b8a-1f677a0543cd"
        className="section"
      >
        <div className="wrapper">
          <div className="columns-8 w-row">
            <div className="w-col w-col-6">
              <h1 className="heading-94">
                <strong>About us</strong> <br />
                Shaping the Future of Structural Engineering, Delivering
                Excellence in Design
              </h1>
            </div>
            <div className="w-col w-col-6">
              <div className="text-block-51">
                Collaboration is in our DNA, and we strive to succeed together.
                Our team of experts delivers reliable and detail-oriented
                solutions for your project. From the first handshake to the final
                beam, superior service guides our every step. Explain who’s behind
                it, what it does, and what makes it unique. Share its core values
                and what this site has to offer
              </div>
            </div>
          </div>
          <div className="tripple-grid">
            <div className="div-block-13">
              <h1 className="heading-102">Meet The Team</h1>
              <div className="div-block-18">
                <div className="div-block-20">
                  <div className="image-team">
                    <img
                      src="images/maf.avif"
                      loading="lazy"
                      alt="madhan"
                      className="image-71"
                    />
                  </div>
                  <h1 className="heading-83">
                    <strong className="bold-text-19">
                      Madhan Kumar K, MS, P.E
                    </strong>
                  </h1>
                  <div className="text-block-37">Partner</div>
                  <a
                    href="https://www.linkedin.com/in/madhankumark-ms-pe/"
                    className="link-block w-inline-block"
                  >
                    <img
                      src="images/linkedin.svg"
                      loading="lazy"
                      alt="Linkedin"
                      className="image-51"
                    />
                  </a>
                </div>
                <div className="div-block-21">
                  <div className="image-team">
                    <img
                      src="images/sri.avif"
                      loading="lazy"
                      alt="sridhar"
                      className="image-72"
                    />
                    <h1 className="heading-84">
                      <strong className="bold-text-20">Sridhar Kethu P.E</strong>
                    </h1>
                  </div>
                  <div className="text-block-36">Partner</div>
                  <a
                    href="https://www.linkedin.com/in/sridhar-kethu-62997a166/"
                    className="link-block-2 w-inline-block"
                  >
                    <img
                      src="images/linkedin.svg"
                      loading="lazy"
                      alt="Linkedin"
                      className="image-52"
                    />
                  </a>
                </div>
                <div className="div-block-22">
                  <div className="image-team">
                    <img
                      src="images/Venkatesh.jpeg"
                      loading="lazy"
                      alt="venkatesh"
                      className="image-73"
                    />
                  </div>
                  <h1 className="heading-85">
                    <strong className="bold-text-21">Venkatesh Kannan</strong>
                  </h1>
                  <div className="text-block-53">Structural Engineer</div>
                  <a
                    href="https://www.linkedin.com/in/venkatesh-kannan-b52311b4/"
                    className="link-block-3 w-inline-block"
                  >
                    <img
                      src="images/linkedin.svg"
                      loading="lazy"
                      alt="Linkedin"
                      className="image-53"
                    />
                  </a>
                </div>
                <div className="div-block-31">
                  <div className="image-team">
                    <img
                      src="images/SAANVI-STRUCTURAL-SOLN-3-1.png"
                      loading="lazy"
                      alt="saanvi"
                      className="image-74"
                    />
                  </div>
                  <h1 className="heading-87">
                    <strong className="bold-text-21">Selvamuthumani T</strong>
                  </h1>
                  <div className="text-block-54">Structural Engineer</div>
                  <a
                    href="https://www.linkedin.com/in/selvamuthumani-t-345355179/"
                    className="link-block-4 w-inline-block"
                  >
                    <img
                      src="images/linkedin.svg"
                      loading="lazy"
                      width={23}
                      alt="Linkedin"
                      className="image-54"
                    />
                  </a>
                </div>
              </div>
              <div className="div-block-19">
                <div className="div-block-32">
                  <img
                    src="images/kell.avif"
                    loading="lazy"
                    alt="kelli"
                    className="image-50"
                  />
                  <h1 className="heading-89">
                    <strong className="bold-text-23">
                      Kelli LeVasseur, P.E.
                    </strong>
                  </h1>
                  <div>Partner</div>
                  <img
                    src="images/linkedin.svg"
                    loading="lazy"
                    alt="Linkedin"
                    className="image-55"
                  />
                </div>
                <div className="div-block-32">
                  <img
                    src="images/kell.avif"
                    loading="lazy"
                    alt="kelli"
                    className="image-50"
                  />
                  <h1 className="heading-89">
                    <strong className="bold-text-23">
                      Kelli LeVasseur, P.E.
                    </strong>
                  </h1>
                  <div>Partner</div>
                  <img
                    src="images/linkedin.svg"
                    loading="lazy"
                    alt="Linkedin"
                    className="image-55"
                  />
                </div>
                <div className="div-block-32">
                  <img
                    src="images/kell.avif"
                    loading="lazy"
                    alt="kelli"
                    className="image-50"
                  />
                  <h1 className="heading-89">
                    <strong className="bold-text-23">
                      Kelli LeVasseur, P.E.
                    </strong>
                  </h1>
                  <div>Partner</div>
                  <img
                    src="images/linkedin.svg"
                    loading="lazy"
                    alt="Linkedin"
                    className="image-55"
                  />
                </div>
                <div className="div-block-33">
                  <img
                    src="images/jenni.jpg"
                    loading="lazy"
                    alt="jenni"
                    className="image-50"
                  />
                  <h1 className="heading-90">
                    <strong>Jennifer Sheppard, P.E.</strong>
                  </h1>
                  <div>Associate Partner, Forensics</div>
                  <img
                    src="images/linkedin.svg"
                    loading="lazy"
                    alt="Linkedin"
                    className="image-56"
                  />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div
        data-w-id="350f89c8-a0a5-5fe9-c5c1-43f430638088"
        className="section video-section"
      >
        <div className="wrapper" />
        <div className="background-cover">
          <div className="background-banner team-banner">
            <div className="image-overlay dark-overylay" />
          </div>
        </div>
        <div style={{ height: "100%" }} className="section-bottom-cover">
          <div className="stripe-wrapper">
            <div className="wrapper no-padding">
              <div className="stripe-container">
                <div className="pin-stripe hide-horizontal-mobile" />
                <div className="pin-stripe" />
              </div>
            </div>
          </div>
        </div>
      </div>
      <div
        data-w-id="6ba567b0-a54f-0ab6-65d6-ff29e51fa4b6"
        className="section no-bottom-padding"
      >
        <div className="wrapper">
          <div className="banner-wrapper">
            <div className="image-cover-wrapper">
              <div style={{ width: "100%" }} className="image-cover" />
            </div>
          </div>
        </div>
        <div className="wrapper">
          <div className="project-categories-wrapper">
            <div style={{ height: 0 }} className="banner-categories">
              <div className="category-collection-list-wrapper w-dyn-list">
                <div role="list" className="categories-grid w-dyn-items">
                  <div
                    role="listitem"
                    className="category-collection-item w-dyn-item"
                  >
                    <div className="card-hover-item">
                      <div className="card-circle white" />
                      <a href="#" className="category-link w-inline-block">
                        <div
                          style={{
                            WebkitTransform:
                              "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                            MozTransform:
                              "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                            msTransform:
                              "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                            transform:
                              "translate3d(0, 100%, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 10deg)",
                            opacity: 0
                          }}
                          className="subtitle w-dyn-bind-empty"
                        />
                      </a>
                    </div>
                  </div>
                </div>
                <div className="w-dyn-empty">
                  <div>No items found.</div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div
        data-w-id="c6a86f47-145d-b8c8-1c44-2b43c426b91d"
        className="footer-section"
      >
        <div className="wrapper">
          <img
            src="images/SAANVI-STRUCTURAL-SOLN-3-1.png"
            loading="lazy"
            alt="saanvi"
          />
          <div className="footer-wrapper">
            <div
              id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b923-c426b91d"
              className="stacked-description"
            >
              <div className="subtitle white">Follow us</div>
              <div className="service-menu-list white">
                <a
                  id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b92b-c426b91d"
                  href="#"
                  target="_blank"
                  className="menu-list-item white small w-inline-block"
                >
                  <div className="team-social-icon small">
                    <img src="images/facebook-f.svg" loading="lazy" alt="" />
                  </div>
                  <div
                    id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b92d-c426b91d"
                    className="text-block-20"
                  >
                    @Saanvi
                  </div>
                  <img
                    src="images/arrow-right48x482x.svg"
                    loading="lazy"
                    id="w-node-_3ba5fc0d-00d1-0032-83ee-52ea90be55fb-c426b91d"
                    alt="arrow"
                    className="list-item-arrow"
                  />
                </a>
                <a
                  href="https://www.linkedin.com/company/saanvi-structural-solutions/"
                  target="_blank"
                  className="menu-list-item white small w-inline-block"
                >
                  <div className="team-social-icon small">
                    <img
                      src="images/linkedin.svg"
                      loading="lazy"
                      alt="Linkedin"
                    />
                  </div>
                  <div
                    id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b931-c426b91d"
                    className="text-block-21"
                  >
                    @Saanvi
                  </div>
                  <img
                    src="images/arrow-right48x482x.svg"
                    loading="lazy"
                    id="w-node-d7478ca7-7861-03a0-e2af-d807b17707c6-c426b91d"
                    alt="arrow"
                    className="list-item-arrow"
                  />
                </a>
              </div>
            </div>
            <div
              id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b937-c426b91d"
              className="stacked-description"
            >
              <div className="subtitle white">Navigation</div>
              <div className="service-menu-list white">
                <a
                  href="team.html"
                  aria-current="page"
                  className="menu-list-item white w-inline-block w--current"
                >
                  <div
                    id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b93c-c426b91d"
                    className="list-item-text"
                  >
                    Team
                  </div>
                  <img
                    src="images/arrow-right48x482x.svg"
                    loading="lazy"
                    id="w-node-eaf30d13-d8b4-446b-d7d2-d527e95fa99a-c426b91d"
                    alt="arrow"
                    className="list-item-arrow"
                  />
                </a>
                <a
                  href="projects.html"
                  className="menu-list-item white w-inline-block"
                >
                  <div
                    id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b93f-c426b91d"
                    className="list-item-text"
                  >
                    Projects
                  </div>
                  <img
                    src="images/arrow-right48x482x.svg"
                    loading="lazy"
                    id="w-node-_6e84a20c-5fdf-34ef-f5da-11af0219cea5-c426b91d"
                    alt="arrow"
                    className="list-item-arrow"
                  />
                </a>
                <a
                  href="services.html"
                  className="menu-list-item white w-inline-block"
                >
                  <div
                    id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b942-c426b91d"
                    className="list-item-text"
                  >
                    Services
                  </div>
                  <img
                    src="images/arrow-right48x482x.svg"
                    loading="lazy"
                    id="w-node-_800e5816-2ed2-cae5-269d-04a046bc20b0-c426b91d"
                    alt="arrow"
                    className="list-item-arrow"
                  />
                </a>
                <a
                  href="contact.html"
                  className="menu-list-item white w-inline-block"
                >
                  <div
                    id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b948-c426b91d"
                    className="list-item-text"
                  >
                    Contact
                  </div>
                  <img
                    src="images/arrow-right48x482x.svg"
                    loading="lazy"
                    id="w-node-f9bea813-e527-c7ca-8e20-11a3cd4098db-c426b91d"
                    alt="arrow"
                    className="list-item-arrow"
                  />
                </a>
              </div>
            </div>
            <div
              id="w-node-c6a86f47-145d-b8c8-1c44-2b43c426b94a-c426b91d"
              className="about-description"
            >
              <h4 className="heading-15">About us</h4>
              <div className="body-display white">
                Shaping the Future of Saanvi Structural Engineering, Delivering
                Excellence in Design.
                <br />
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <Footer/>
  </>
  
  )
}

export default Team
